class Solution(object):
    ...
    def minPath(self, grid,i=0,j=0):
        ...
        if i == n-1 and j == m-1:
            ...
            return grid[i][j] + self.minPath(grid,i,j+1)
        elif j == m-1:
            return grid[i][j] + self.minPath(grid,i+1,j)
        else:
            return grid[i][j] + min(self.minPath(grid,i+1,j),self.minPath(grid,i,j+1))       
